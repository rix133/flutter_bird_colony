      ...
      child: MaterialApp(
        ...
        onGenerateRoute: (settings) {
          ...
          if (settings.name == '/editNest') {
            return MaterialPageRoute(
              builder: (context) => EditNest(firestore: firestore),
              settings: settings,
            );
          }
          if (settings.name == '/editBird') {
            return MaterialPageRoute(
              builder: (context) => EditBird(firestore: firestore),
              settings: settings,
            );
          }
  ...
  group("nest experiment options", () {
    ...
    testWidgets("can save a new experiment", (WidgetTester tester) async {
      ...
      List<Experiment> experiments =
          docs.map((e) => Experiment.fromDocSnapshot(e)).toList();
      //get the new experiment
      Experiment? newExperiment = experiments
  ...
  group("bird experiment options", () {
    ...
    setUp(() async {
      experiment = Experiment(
          ...
          birds: ["AA1235"],
          measures: []);
      experiment.previousBirds = [
        "AA1235"
      ]; //set this manually because it is not saved to firestore
      chick.experiments = [experiment];
      await firestore.collection("Birds").doc(chick.band).set(chick.toJson());
      await firestore
          .collection('experiments')
          .doc(experiment.id)
    ...
    testWidgets("can go to a Bird editing", (WidgetTester tester) async {
      myApp = getInitApp(experiment);
      await tester.pumpWidget(myApp);
      await tester.pumpAndSettle();
    ...
    testWidgets("can add new bird to experiment", (WidgetTester tester) async {
      myApp = getInitApp(experiment);
      await tester.pumpWidget(myApp);
      await tester.pumpAndSettle();
      //searhctext new Experiment
      ...
      //search button new nests and tap it
      await tester.tap(find.text("Select birds"));
      await tester.pumpAndSettle();
      ...
      expect(find.text("Bird ID: AA1234"), findsOneWidget);
    });
  });
}
