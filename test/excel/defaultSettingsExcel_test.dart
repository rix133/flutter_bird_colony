...
void main() {
  group('DefaultSettings', () {
    test('toExcelRowHeader should return correct headers', () {
      ...
      expect(headers[1].value.text, 'Auto next band');
      expect(headers[2].value.text, 'Auto next band parent');
      expect(headers[3].value.text, 'Default location');
      expect(headers[4].value.text, 'Biased repeated measurements');
      expect(headers[5].value.text, 'Default species');
    ...
    test('toExcelRows should return correct rows', () async {
      ...
      expect(
          (rows[0][6] as TextCellValue).value.text, settings.responsible ?? '');
    });
  });
}
